...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  await addAuditLog(user.name, roleLabel(user.role), `Updated a PDS reference for ${employee.displayName} (#${employee.employeeNumber})`);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  await addAuditLog(user.name, roleLabel(user.role), `Removed a PDS reference for ${employee.displayName} (#${employee.employeeNumber})`);
