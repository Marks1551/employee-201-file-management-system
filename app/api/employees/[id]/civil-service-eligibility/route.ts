...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  await addCivilServiceEligibility(id, data);
  await addAuditLog(user.name, roleLabel(user.role), `Added a civil service eligibility record for ${employee.displayName} (#${employee.employeeNumber})`);
