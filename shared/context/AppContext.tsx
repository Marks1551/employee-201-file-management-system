...
import { roleLabel, roleHome } from '@/shared/lib/roles';
import type {
  Employee, User, AuditLogEntry, Notification, AppMeta, Role, EmployeeStatus, EmployeeInput,
} from '@/shared/types';
...
export interface AppContextValue {
  ...
  completeAccountSetup: (token: string, password: string) => Promise<{ ok: true; user: User } | { ok: false; error: string }>;
  ...
  addUserInvite: (data: { employeeNumber?: string; name?: string; email?: string; role: Role }) => Promise<ActionResult>;
  ...
  submitDocument: (employeeId: string | null | undefined, docTypeName: string, docFile?: File | null) => Promise<ActionResult>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (identifier: string, password: string) => {
    try {
      const { user } = await api<{ user: User }>('/api/auth/login', { method: 'POST', body: { identifier, password } });
      setCurrentUser(user);
      await refreshAll();
  ...
  const completeAccountSetup = useCallback(async (token: string, password: string) => {
    try {
      const { user } = await api<{ user: User }>('/api/auth/account-setup', { method: 'POST', body: { token, password } });
      setCurrentUser(user);
      await refreshAll();
      return { ok: true as const, user };
    } catch (err) {
  ...
  const resendInvite = useCallback(async (userId: string): Promise<ActionResult> => {
    try {
  ...
  // ---------- users / accounts ----------
  const addUser = useCallback(async (data: Record<string, unknown>) => {
    try {
  ...
  const addUserInvite = useCallback(async (data: { employeeNumber?: string; name?: string; email?: string; role: Role }): Promise<ActionResult> => {
    try {
  ...
  const updateUser = useCallback(async (id: string, patch: Record<string, unknown>) => {
    try {
  ...
  const changePassword = useCallback(async (userId: string, currentPassword: string, newPassword: string): Promise<ActionResult> => {
    try {
  ...
  // ---------- employees ----------
  const addEmployee = useCallback(async (data: EmployeeInput) => {
    try {
  ...
  const setEmployeeStatus = useCallback(async (id: string, status: EmployeeStatus, reason?: string | null): Promise<ActionResult> => {
    try {
      const { employee } = await api<{ employee: Employee }>(`/api/employees/${id}`, { method: 'PATCH', body: { status, deactivationReason: reason ?? null } });
      setEmployees((list) => list.map((e) => (e.id === id ? employee : e)));
      return { ok: true };
    } catch (err) {
      return { ok: false, error: errorMessage(err) };
   ...
   *  Returns { ok, error? }. */
  const uploadDocument = useCallback(async (employeeId: string, docId: string, docFile?: File | null): Promise<ActionResult> => {
    try {
   ...
   *  Returns { ok, error? }. */
  const rejectDocument = useCallback(async (employeeId: string, docId: string, note?: string): Promise<ActionResult> => {
    try {
  ...
  const submitDocument = useCallback(async (employeeId: string | null | undefined, docTypeName: string, docFile?: File | null): Promise<ActionResult> => {
    try {
      const body = new FormData();
  ...
  const addTraining = useCallback(async (employeeId: string, data: Record<string, unknown>): Promise<ActionResult> => {
    try {
      const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/trainings`, { method: 'POST', body: data });
  ...
  const updateTraining = useCallback(async (employeeId: string, trainingId: string, patch: Record<string, unknown>): Promise<ActionResult> => {
    try {
      const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/trainings/${trainingId}`, { method: 'PATCH', body: patch });
      setEmployees((list) => list.map((e) => (e.id === employeeId ? employee : e)));
      return { ok: true };
    } catch (err) {
      return { ok: false, error: errorMessage(err) };
  ...
  const deleteTraining = useCallback(async (employeeId: string, trainingId: string): Promise<ActionResult> => {
    try {
      const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/trainings/${trainingId}`, { method: 'DELETE' });
      setEmployees((list) => list.map((e) => (e.id === employeeId ? employee : e)));
      return { ok: true };
    } catch (err) {
      return { ok: false, error: errorMessage(err) };
  ...
  const uploadTrainingCertificate = useCallback(async (employeeId: string, trainingId: string, certFile: File): Promise<ActionResult> => {
    try {
      const body = new FormData();
      body.append('file', certFile);
      const res = await fetch(`/api/employees/${employeeId}/trainings/${trainingId}/certificate`, { method: 'POST', body });
      const data = await res.json().catch(() => ({}));
  ...
  const removeTrainingCertificate = useCallback(async (employeeId: string, trainingId: string): Promise<ActionResult> => {
    try {
      const res = await fetch(`/api/employees/${employeeId}/trainings/${trainingId}/certificate`, { method: 'DELETE' });
      const data = await res.json().catch(() => ({}));
  ...
  function makeRecordActions(basePath: string): RecordActions<Record<string, unknown>> {
    const add = async (employeeId: string, data: Record<string, unknown>): Promise<ActionResult> => {
      try {
        const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/${basePath}`, { method: 'POST', body: data });
        setEmployees((list) => list.map((e) => (e.id === employeeId ? employee : e)));
        return { ok: true };
      } catch (err) {
        return { ok: false, error: errorMessage(err) };
    ...
    const update = async (employeeId: string, recordId: string, patch: Record<string, unknown>): Promise<ActionResult> => {
      try {
        const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/${basePath}/${recordId}`, { method: 'PATCH', body: patch });
        setEmployees((list) => list.map((e) => (e.id === employeeId ? employee : e)));
        return { ok: true };
      } catch (err) {
        return { ok: false, error: errorMessage(err) };
    ...
    const remove = async (employeeId: string, recordId: string): Promise<ActionResult> => {
      try {
        const { employee } = await api<{ employee: Employee }>(`/api/employees/${employeeId}/${basePath}/${recordId}`, { method: 'DELETE' });
        setEmployees((list) => list.map((e) => (e.id === employeeId ? employee : e)));
        return { ok: true };
      } catch (err) {
        return { ok: false, error: errorMessage(err) };
  ...
  const backupNow = useCallback(async () => {
    const { stamp } = await api<{ stamp: string }>('/api/backup', { method: 'POST' });
  ...
  const markNotificationRead = useCallback(async (id: string) => {
    try {
      const { notifications: fresh } = await api<{ notifications: Notification[] }>(`/api/notifications/${id}`, { method: 'PATCH' });
      setNotifications(fresh);
    } catch (err) {
  ...
  const markAllNotificationsRead = useCallback(async () => {
    try {
      const { notifications: fresh } = await api<{ notifications: Notification[] }>('/api/notifications/read-all', { method: 'POST' });
      setNotifications(fresh);
    } catch (err) {
